...
const FeedCard: React.FC<{ item: FeedItem }> = ({ item }) => {
      ...
      case TaskType.RUNNING:
        ...
        return (
          <div className="space-y-3">
             {runningItem.description && <p className="text-sm text-white/80 py-1">{runningItem.description}</p>}
            <div className="flex items-start justify-between my-2">
              {/* Left side: Distance */}
              <div className="text-left">
                <p className="text-3xl md:text-4xl font-bold text-white tracking-tight leading-none drop-shadow-lg">
              ...
              <div className="text-right space-y-1.5">
                <div>
                  <p className="text-lg font-semibold text-white leading-tight drop-shadow">
            ...
            {runningItem.pathPoints && runningItem.pathPoints.length > 1 ? (
              <div className="mt-3">
                <RunPathMap pathPoints={runningItem.pathPoints} />
                 ...
                 <div className="h-auto py-3 flex items-center justify-center text-white/60 text-xs glass rounded-xl border border-white/20 mt-3">
      ...
      case TaskType.WAKE_UP:
        ...
        return (
          <div className="mt-3 space-y-3">
             {wakeUpItem.description && <p className="text-sm text-white/80 py-1">{wakeUpItem.description}</p>}
            {wakeUpItem.videoBlob && videoSrc ? (
              <div
                className="aspect-square glass rounded-xl overflow-hidden relative group cursor-pointer"
              ...
              >
                ...
                >
                  <div className="glass-button p-4 rounded-full text-white">
               ...
               <div className="aspect-square glass rounded-xl flex items-center justify-center text-white/60"> 
                Video yuklanmagan
              </div>
            )}
          </div>
      ...
      case TaskType.DAILY_PLAN:
        ...
        const renderTodoItem = (todo: TodoItem, plan: DailyPlanFeedItem) => {
          ...
          return (
            <div 
              key={todo.id} 
              className={`flex items-center space-x-2 p-3 rounded-xl glass border border-white/20 focus:outline-none focus:ring-1 focus:ring-white/50
                          ${todo.isMandatory ? 'bg-white/5' : 'bg-white/10'}
                          ${canInteractWithTodo ? 'cursor-pointer hover:bg-white/20' : (isCurrentUserItemOwner ? 'opacity-70 cursor-default' : 'opacity-60 cursor-not-allowed')}
            ...
            >
              {todo.isCompleted 
                ? <CheckCircleIcon className="w-4 h-4 flex-shrink-0 text-green-400" /> 
                : <CircleIcon className="w-4 h-4 text-white/40 flex-shrink-0" />
              }
              <span className={`flex-grow ${todo.isCompleted ? 'line-through text-white/50' : 'text-white'}`}>
                {todo.text}
              </span>
              {todo.isMandatory && (
                <span className="text-[10px] text-white/50 font-medium ml-auto">(avtomatik)</span>
              )}
            </div>
          );
        };
        ...
        return (
          <div className="mt-3">
            <h4 className="font-semibold text-base text-white mb-3 drop-shadow">
                {UZBEK_STRINGS.planForDate(planDateStr)}
            </h4>
            <div className="space-y-2"> 
              {displayedMandatoryTodos.length > 0 && (
                <>
                  <p className="text-[10px] font-medium text-white/60 uppercase tracking-wider mt-1 mb-1">{UZBEK_STRINGS.mandatoryTasks}</p>
                  {displayedMandatoryTodos.map(todo => renderTodoItem(todo, dailyPlanItem))}
                </>
              )}
              {displayedPersonalTodos.length > 0 && (
                 <>
                  <p className="text-[10px] font-medium text-white/60 uppercase tracking-wider mt-2 mb-1">{UZBEK_STRINGS.personalTasks}</p>
                  {displayedPersonalTodos.map(todo => renderTodoItem(todo, dailyPlanItem))}
                 </>
              )}
              {allTodosInPlan.length === 0 && ( 
                <p className="text-xs text-white/60 py-2">{UZBEK_STRINGS.noTodosYet}</p>
              )}
            </div>
            {canToggleExpansion && (
              <button
                onClick={() => setIsPlanExpanded(prev => !prev)}
                className="glass-button text-xs text-white/80 hover:text-white mt-3 p-2 w-full text-left font-medium rounded-xl transition-all duration-300"
              >
                {isPlanExpanded
                  ? 'Kamroq ko\'rsatish'
        ...
        return (
          <div className="mt-3 flex items-start space-x-4">
            ...
            <div className="flex-grow min-w-0 space-y-2">
                <p className="text-md font-semibold text-white drop-shadow">"{bookItem.bookTitle}"</p>
                <p className="text-sm text-white/80">
                    <span className="font-medium">{bookItem.pagesRead}</span> sahifa o'qildi.
                </p>
                {bookItem.review && (
                <div 
                    className="text-xs text-white/70 pt-1 italic cursor-pointer"
                    onClick={() => setIsReviewExpanded(!isReviewExpanded)}
                >
                    <p className={`${!isReviewExpanded && 'line-clamp-4'}`}>
                    <span className="font-semibold not-italic text-white/90">Sharh: </span>{bookItem.review}
                    </p>
                    {isReviewLong && (
                    <span className="text-blue-300 font-semibold not-italic hover:underline mt-1 inline-block">
                ...
                {audioSrc && ( 
                    <div className="mt-2 flex items-center space-x-3 glass p-3 rounded-xl border border-white/20">
                        <button onClick={toggleAudioPlayPause} className="glass-button p-2 text-white rounded-full transition-all duration-300">
                            {isAudioPlaying ? <PauseIcon className="w-4 h-4" /> : <PlayIcon className="w-4 h-4" />}
                        </button>
                        <p className="text-xs text-white/80 font-medium">Audio Xulosa</p>
                        <audio
                            ref={audioRef}
                            src={audioSrc}
  ...
  return (
    <div className={`glass-card rounded-2xl p-4 mx-2 shadow-lg
                    ${isOfflineItem && !item.syncFailed ? 'border-l-4 border-l-amber-400' : ''} 
                    ${item.syncFailed ? 'border-l-4 border-l-red-500' : ''}
                    relative floating`}> 
        <div className="flex items-start justify-between">
            <div 
                className="flex items-center space-x-3 mb-3 cursor-pointer flex-grow min-w-0"
            ...
            >
                ...
                alt={item.userName} 
                className="w-10 h-10 rounded-full object-cover border-2 border-white/30 flex-shrink-0 shadow-lg" 
                />
                <div className="min-w-0">
                <h3 className="font-semibold text-sm text-white leading-tight truncate drop-shadow">{item.userName}</h3>
                <p className="text-xs text-white/60 leading-tight flex items-center">
                    {formatFeedTimestamp(new Date(dateToDisplay))}
                    {isOfflineItem && !item.syncFailed && (
                    <span className="ml-1.5 text-amber-300 text-[9px] flex items-center" title={UZBEK_STRINGS.syncingOfflineItem}> 
                        <ClockIcon className="w-2.5 h-2.5 mr-0.5 animate-ping opacity-75" /> 
                        Offline
                    </span>
                    )}
                    {item.syncFailed && (
                    <span className="ml-1.5 text-red-400 text-[9px] flex items-center" title={UZBEK_STRINGS.syncFailedIndicatorTitle}>
            ...
            {isCurrentUserItemOwner && (
                <div ref={menuRef} className="relative flex-shrink-0 -mr-2">
                    <button 
                        ...
                        className="glass-button p-2 text-white/60 hover:text-white rounded-full transition-all duration-300"
                    ...
                    {isMenuOpen && (
                        <div 
                            className="absolute top-full right-0 mt-1 w-36 glass-modal rounded-xl z-20 py-1 shadow-2xl"
                            role="menu"
                        ...
                        >
                            <button
                                ...
                                className="flex items-center w-full text-left px-3 py-2 text-sm text-gray-700 hover:bg-white/10 hover:text-white transition-colors rounded-lg mx-1"
                                role="menuitem"
                            >
                            ...
                            <button
                                ...
                                className="flex items-center w-full text-left px-3 py-2 text-sm text-red-400 hover:bg-red-500/20 hover:text-red-300 transition-colors rounded-lg mx-1"
                                role="menuitem"
                            >
      ...
      <h2 className="text-base font-semibold uppercase text-white mb-2 tracking-tight drop-shadow">
        {item.type === TaskType.RUNNING && (item as RunningFeedItem).title ? (item as RunningFeedItem).title : getActivityTitle(item.type)}
      </h2>

      {renderSpecificContent()}

      <div className="mt-4 pt-3 border-t border-white/20 flex items-center space-x-4">
        <button 
          onClick={handleLikeClick}
          className={`flex items-center space-x-1.5 transition-all duration-300 group glass-button px-3 py-1.5 rounded-full
                      ${isLikedByCurrentUser ? 'text-red-400 bg-red-500/20' : 'text-white/60 hover:text-red-400'}`}
        ...
        <button 
            ...
            className="flex items-center space-x-1.5 text-white/60 hover:text-blue-400 transition-all duration-300 group glass-button px-3 py-1.5 rounded-full" 
        ...
        <span className="text-xs text-white/40 uppercase ml-auto">
      ...
      {showComments && (
        <div className="mt-4 pt-4 border-t border-white/20 space-y-3">
          {item.comments.length === 0 ? (
            <p className="text-xs text-white/60 text-center py-3">{UZBEK_STRINGS.noCommentsYet}</p>
          ) : (
            item.comments.slice().sort((a,b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime()).map(comment => (
              <div key={comment.id} className="flex items-start space-x-2">
                <img 
                  ...
                  className="w-7 h-7 rounded-full object-cover border border-white/30 mt-0.5"
                />
                <div className="flex-1 glass p-3 rounded-xl text-xs border border-white/20">
                  <div className="flex items-baseline justify-between">
                    <span className="font-semibold text-white">{comment.userName}</span>
                    <span className="text-white/50 text-[10px]">{formatRelativeTime(new Date(comment.timestamp))}</span>
                  </div>
                  <p className="text-white/80 mt-0.5 whitespace-pre-wrap break-words">{comment.text}</p>
                </div>
              </div>
            ))
          )}
          <div className="flex items-center space-x-2">
            <input 
              ...
              className="flex-grow glass-input text-white text-xs rounded-xl p-3 placeholder-white/50"
            ...
            <button 
              ...
              className="glass-button p-3 text-white rounded-xl transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
      ...
      {confirmation.show && (
        ...
        > 
          <div className="glass-modal p-6 rounded-2xl shadow-2xl max-w-sm w-full"> 
            ...
            <div className="flex justify-end space-x-3">
              ...
              <button
                ...
                className="px-4 py-2 text-sm font-medium text-white bg-gradient-to-r from-green-500 to-green-600 rounded-xl transition-all duration-300 hover:from-green-600 hover:to-green-700 shadow-lg"
              >
                {UZBEK_STRINGS.yes}
              </button>
            </div>
